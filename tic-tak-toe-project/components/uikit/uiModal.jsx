  ...
  const modal = (
    ...
    >
      <div
        data-id="modal"
        className={clsx(
          "flex flex-col bg-white rounded-lg min-h-80 mx-auto relative",
          {
      ...
      >
        <button
          ...
          className="
            ...
            hover:bg-white/30 bg-white/10 transition-colors
...
UiModal.Header = function UiModalHeader({ children, className }) {
  return (
    <div className={clsx(className, "px-6 pt-6 pb-4 text-2xl font-bold")}>
      {children}
    </div>
  );
};
...
UiModal.Footer = function UiModalFooter({ children, className }) {
  return (
    <div className={clsx(className, "mt-auto p-6 gap-4 flex justify-end")}>
      {children}
    </div>
  );
};
